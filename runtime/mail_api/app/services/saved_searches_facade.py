...
class SavedSearchesFacade:
    ...
    async def create_saved_search(self, user_id: str, payload: SavedSearchCreateRequest, tenant_id: str = "default", account_id: str = "") -> SavedSearchDoc:
        ss = SavedSearchDoc(user_id=user_id, tenant_id=tenant_id, account_id=account_id or user_id, name=payload.name, query=payload.query, filters=payload.filters)
        await ss.insert()
        return ss
